...
**路径别名**：`@/` 映射到 `src/`（见 [tsconfig.json](file:///c:/Users/XuShuang/Desktop/demo/frontend/tsconfig.json#L33-L35)）。
...
**参考**：[BaseButton.vue](file:///c:/Users/XuShuang/Desktop/demo/frontend/src/components/ui/BaseButton.vue#L16-L63)
...
**参考**：[BaseModal.vue](file:///c:/Users/XuShuang/Desktop/demo/frontend/src/components/ui/BaseModal.vue#L5-L14)
...
- 触摸目标最小 44px：`min-h-[44px] min-w-[44px]`（参考 [BaseButton.vue](file:///c:/Users/XuShuang/Desktop/demo/frontend/src/components/ui/BaseButton.vue#L67-L69)）
...
**参考**：[auth.ts](file:///c:/Users/XuShuang/Desktop/demo/frontend/src/stores/auth.ts)
...
| Store | 文件 | 职责 |
|-------|------|------|
| `useAuthStore` | [stores/auth.ts](file:///c:/Users/XuShuang/Desktop/demo/frontend/src/stores/auth.ts) | 用户认证状态、登录/登出、token 管理、Sentry 用户上下文同步 |
| `useToastStore` | [stores/toast.ts](file:///c:/Users/XuShuang/Desktop/demo/frontend/src/stores/toast.ts) | 全局 Toast 消息队列 |
...
**参考**：[router/index.ts](file:///c:/Users/XuShuang/Desktop/demo/frontend/src/router/index.ts#L17-L113)
...
项目使用原生 `fetch` 封装（非 axios），核心位于 [services/request.ts](file:///c:/Users/XuShuang/Desktop/demo/frontend/src/services/request.ts)。
...
**参考**：[auth.ts](file:///c:/Users/XuShuang/Desktop/demo/frontend/src/services/modules/auth.ts)、[course.ts](file:///c:/Users/XuShuang/Desktop/demo/frontend/src/services/modules/course.ts)
...
```ts
export { authAPI, userAPI, courseAPI, ... } from './modules/...'
export { saveAuth, getAuth, clearAuth, startHeartbeat, stopHeartbeat } from './request'
export default { authAPI, userAPI, ... } // 兼容旧代码
```
...
```ts
// vite.config.ts + tsconfig 自动解析，无需额外插件
...
- Bundle 分析（`rollup-plugin-visualizer`，输出 `stats.html`）
...
props.value = 'new value' // ❌ 直接修改 props，违反单向数据流
</script>
```
...
Vue 3 中 `v-if` 优先级高于 `v-for`，二者同元素会导致 `v-if` 无法访问 `v-for` 作用域变量。**NEVER** 在同一元素上同时使用。
...
在 `<script setup>` 中，**NEVER** 直接使用 `$emit` 而不通过 `defineEmits` 声明。未声明的 emit 在 TypeScript 严格模式下类型不安全，且无法获得 IDE 智能提示。
...
function handleClick() {
  $emit('submit', formData) // TypeScript 报错：$emit 不存在
}
</script>
```
...
function handleClick() {
  emit('submit', formData) // ✅ 类型安全，IDE 有完整提示
}
</script>
```
...
使用 `async setup()` 或顶层 `await` 的异步组件，**NEVER** 在没有 `<Suspense>` 包裹的情况下直接渲染，否则会导致组件树悬挂，用户看不到任何内容直到异步操作完成。

**❌ 错误示例：**

```vue
<script setup lang="ts">
// ❌ 顶层 await，无 Suspense 包裹 → 父组件渲染被阻塞
...
<template>
  <Suspense>
    ...
    <template #fallback>
      <LoadingSkeleton /> <!-- ✅ 异步期间展示 fallback -->
    </template>
  </Suspense>
</template>

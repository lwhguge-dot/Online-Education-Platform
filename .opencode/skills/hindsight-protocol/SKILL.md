...
description: Hindsight 记忆系统协议。AI 每轮必须手动执行 recall/retain/session_end 流程。
---

# Hindsight Memory Protocol

> **MiMoCode 注意：** `hindsight-automemory.ts` 插件在 MiMoCode 中不会自动加载（MiMoCode 不支持本地 TS 插件）。所有操作必须由 AI 手动执行。

## 每轮必须执行的流程

1. **代码修改后** → 立即调用 `hindsight_retain_batch`
2. **用户偏好/纠正时** → 立即调用 `hindsight_retain`
3. **会话结束前** → 调用 `hindsight_session_end`
